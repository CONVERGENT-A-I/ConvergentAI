...
// ── Extractor client ─────────────────────────────────────────────────
// Self-hosted GKE+A100 vLLM (Gemma 4) — same backend as the main
// conversational LLM (see agent.ts). The extractor previously used a
// SEPARATE Cerebras account/API key (cerebrasExtractorApiKey) specifically
// to get its own isolated request pool with zero queue contention against
// the main LLM. That isolation no longer exists now that both paths hit
// the same vLLM deployment:
//   - extractProfileField/extractMultipleFields run CONCURRENTLY with the
//     main LLM turn (see the [perf] logs below — called from onUserTurn,
//     raced against a timeout while the main pipeline is also in flight).
//   - Both now share the same vLLM pod(s)' request queue and KV-cache
//     budget. Under load this can add queueing latency to extraction
//     (and vice versa) that didn't exist with a separate Cerebras pool.
//   - The GKE Inference Gateway (InferencePool) mitigates this somewhat by
//     routing to whichever pod has more free queue/KV-cache headroom, but
//     it does not eliminate contention the way a fully separate provider did.
// If extraction latency regresses noticeably, worth revisiting (e.g. a
// dedicated extractor replica, or a smaller/faster self-hosted model).
const fastClient = new OpenAI({
  apiKey: ailanaConfig.vllmApiKey,
  baseURL: ailanaConfig.vllmBaseUrl,
});

// Must match the --served-model-name the vLLM Deployment/StatefulSet is
// running (see 10-gemma4-vllm.yaml). Previously 'gpt-oss-120b' (Cerebras-only
// model, not available on the self-hosted server).
const EXTRACTION_MODEL = ailanaConfig.vllmModel;
...
User input: "${userInput}"`;
  ...
  try {
    // Retry once for transient vLLM errors
    let llmErr: any = null;
    for (let attempt = 0; attempt < 2; attempt++) {
      try {
        const _perfLlmCallStart = performance.now();
        ...
        const _perfLlmCallMs = (performance.now() - _perfLlmCallStart).toFixed(1);
        console.log(`[perf] llm-extractor extractProfileField("${fieldName}"): vLLM call (attempt ${attempt + 1}) took ${_perfLlmCallMs}ms`);
    ...
    const statusCode = error?.status ?? error?.statusCode;
    console.error(`[llm-extractor] vLLM failed for "${fieldName}" (status: ${statusCode}):`, error?.message ?? error);
    if (statusCode === 400) {
  ...
  try {
    ...
    for (let attempt = 0; attempt < 2; attempt++) {
      try {
        ...
        const _perfLlmCallMs = (performance.now() - _perfLlmCallStart).toFixed(1);
        console.log(`[perf] llm-extractor extractMultipleFields([${_fieldNames}]): vLLM call (attempt ${attempt + 1}) took ${_perfLlmCallMs}ms`);
        content = response.choices[0]?.message?.content || null;
    ...
    const statusCode = error?.status ?? error?.statusCode;
    console.error(`[llm-extractor] vLLM failed for multi-field extraction [${fields.map(f => f.name).join(', ')}] (status: ${statusCode}):`, error?.message ?? error);
    if (statusCode === 400) {
  ...
  try {
    ...
    for (let attempt = 0; attempt < 2; attempt++) {
      try {
        ...
        const _perfLlmCallMs = (performance.now() - _perfLlmCallStart).toFixed(1);
        console.log(`[perf] llm-extractor classifyConfirmation("${fieldName}"): vLLM call (attempt ${attempt + 1}) took ${_perfLlmCallMs}ms`);
        content = response.choices[0]?.message?.content || null;
    ...
    const statusCode = error?.status ?? error?.statusCode;
    console.error(`[llm-extractor] vLLM classify failed for "${fieldName}" (status: ${statusCode}):`, error?.message ?? error);
    if (statusCode === 400) {
