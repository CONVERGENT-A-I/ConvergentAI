...
import { MessageCircle, Sparkles, Send, VolumeX, Bot, History } from "lucide-react";
import { type ChatEntry } from "../../lib/session-storage";

interface InRoomChatPanelProps {
  isActive?: boolean;
  onTriggerLoanOfficer?: () => void;
  /** Pre-loaded transcript entries from localStorage — shown above live messages */
  initialTranscript?: ChatEntry[];
  /** Fired whenever a new live message arrives so the parent can persist it */
  onNewMessage?: (entry: ChatEntry) => void;
}

export function InRoomChatPanel({ isActive, onTriggerLoanOfficer, initialTranscript, onNewMessage }: InRoomChatPanelProps) {
  ...
  useEffect(() => {
    ...
    if (displayMessages.length > startIdx) {
      for (let i = startIdx; i < displayMessages.length; i++) {
        const m = displayMessages[i];
        if (m.final) {
          onNewMessage({
            id: String(m.id),
            role: m.isAgent ? "agent" : "user",
            text: m.text,
  ...
  return (
    <div className="flex flex-col h-full bg-[#050505] rounded-xl md:rounded-none overflow-hidden">
      ...
      >
        ...
        {initialTranscript && initialTranscript.length > 0 && (
          <>
            ...
            {initialTranscript.map((entry, i) => (
              <div
                key={`hist-${entry.id}-${i}`}
                className={`flex gap-2.5 max-w-[90%] opacity-60 ${
                  entry.role === "agent" ? "mr-auto" : "ml-auto flex-row-reverse"
                }`}
              >
                {entry.role === "agent" && (
                  <div className="h-7 w-7 rounded-full bg-[#00b4d8]/60 flex items-center justify-center shrink-0 mt-0.5">
                    <Sparkles className="h-3.5 w-3.5 text-white" />
                  </div>
                )}
                <div className="flex flex-col gap-0.5">
                  <div
                    className={`px-3.5 py-2.5 text-[13px] leading-relaxed rounded-2xl italic ${
                      entry.role === "agent"
                        ? "bg-white/8 text-gray-300 rounded-tl-sm"
                        : "bg-[#00b4d8]/40 text-gray-200 rounded-tr-sm"
                    }`}
                  >
                    {entry.text}
                  </div>
                  <span className={`text-[10px] text-gray-500 font-medium px-1 ${entry.role === "agent" ? "" : "text-right"}`}>
        ...
        {displayMessages.length === 0 && (!initialTranscript || initialTranscript.length === 0) && (
