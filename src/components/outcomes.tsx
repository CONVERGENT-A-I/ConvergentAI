...
export default function Outcomes() {
  ...
  return (
    <section className="bg-bg-primary min-h-[100svh] flex flex-col justify-center py-20 px-8 md:px-12 lg:px-24 overflow-hidden">
      <motion.div 
      ...
      >
        ...
        {/* Subheader */}
        <motion.p variants={itemVariants} className="text-text-secondary text-lg md:text-xl lg:text-2xl font-medium max-w-4xl mx-auto mb-20 md:mb-24 balance opacity-80">
        ...
        <motion.div variants={itemVariants} className="relative group mx-auto">
          {/* Subtle Outer Glow */}
          <div className="absolute -inset-1 bg-brand-teal/20 rounded-[2.5rem] blur-2xl opacity-0 group-hover:opacity-100 transition-opacity duration-1000" />
          
          <div className="relative bg-bg-card border-2 border-border-subtle group-hover:border-brand-teal/60 rounded-[2.5rem] p-12 md:p-16 lg:p-24 overflow-hidden transition-all duration-500">
            {/* Grid for metrics */}
              ...
              {metrics.map((metric, index) => (
                <motion.div variants={itemVariants} key={index} className="flex flex-col gap-4 text-center lg:text-left max-w-[320px] mx-auto lg:mx-0">
                  <h3 className="text-white text-xl md:text-2xl lg:text-3xl font-black leading-[1.2] tracking-tighter text-balance">
                    ...
                    <span className="text-brand-teal italic font-bold">
                      {metric.stat}
                    </span>
                  </h3>
                </motion.div>
        ...
        <motion.p 
          ...
          className="text-text-muted text-sm md:text-base font-medium max-w-3xl mx-auto mt-12 balance opacity-80"
        >
